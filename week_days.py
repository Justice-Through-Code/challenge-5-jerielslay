...
def favorite_activities():
    ...
    fun_activities = []
    for day in days:
        activity = input(f'What is your favorite on {day}? ')
    ...
    for i in range(len(days)):
        print(f'On {days[i]}s, your favorite activity is to {fun_activities[i]}.')
...
def temp_by_day():
    ...
    for day in days:
        temp = int(input(f"Today is {day}. What is today's temperature? "))
...
def temp_by_day_continuous():
    ...
    temp = int(input(f"What is today's temperature outside? "))
    while(temp < 65):
